...
def simulate_trial_random_block(npatients, ndonors, nblocks, phi, epsilon, beta, phi_a, phi_b, epsilon_a, epsilon_b, beta_a, beta_b):
    # Simulate a trial of 'npatients' in each arm (treatment and control), with a given donor list, epsilon and beta values. 
    # Allocates to a random donor at each block.
    ...
    for block_idx in range(nblocks):
        # Pick a donor at random
        donor_idx = np.random.choice(range(ndonors))
        
        # Draw outcomes for each patient in block
        if donorlist[donor_idx] == 1:
            successes = np.random.binomial(npatients_per_block, epsilon)
        else:
            successes = np.random.binomial(npatients_per_block, beta)
        treatment_successes += successes
        treatment_failures += npatients_per_block - successes
    ...
    return pval, donorlist, [contingency_table[0][0], contingency_table[0][1], contingency_table[1][0], contingency_table[1][1]]
...
with open('contingency_tables_random_block/contingencies_phi_' + str(phi) + '_eps_' + str(epsilon) + '_beta_' + str(beta) + '_npatients_' + str(npatients) + '_ndonors_' + str(ndonors) + '_nblocks_' + str(nblocks) + '.txt', 'w') as fid_cont:
    fid_cont.write('successes_FMT\tsuccesses_placebo\tfailures_FMT\tfailures_placebo\n')
    for i in range(Ntrials):
        print(i)
        pval, donorlist, contingency_table = simulate_trial_random_block(npatients, ndonors, nblocks, phi, epsilon, beta, phi_a, phi_b, epsilon_a, epsilon_b, beta_a, beta_b)
        pvals[i] = pval
        donorlists.append(donorlist)
        fid_cont.write('\t'.join([str(int(cont)) for cont in contingency_table]) + '\n')

with open('pvalues_random_block/pvalues_phi_' + str(phi) + '_eps_' + str(epsilon) + '_beta_' + str(beta)  + '_npatients_' + str(npatients) + '_ndonors_' + str(ndonors) + '_nblocks_' + str(nblocks) + '.txt', 'w') as fid:
    for pval in pvals:
        fid.write(str(pval)+'\n')

with open('donorlists_random_block/donorlists_phi_' + str(phi) + '_eps_' + str(epsilon) + '_beta_' + str(beta) + '_npatients_' + str(npatients) + '_ndonors_' + str(ndonors) + '_nblocks_' + str(nblocks) + '.txt', 'w') as fid:
...
with open('summary_stats_random_block/summary_phi_' + str(phi) + '_eps_' + str(epsilon) + '_beta_' + str(beta) +  '_npatients_' + str(npatients) + '_ndonors_' + str(ndonors) + '_nblocks_' + str(nblocks) + '.txt', 'w') as fid:
